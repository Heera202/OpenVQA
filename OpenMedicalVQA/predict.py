...
def eval_gpt_open_ended(model, dataset, args, print_vis_token_meaning=True):
  ...
  with tqdm(total=len(dataset)) as epoch_pbar:
    ...
    for item in range(len(dataset)):
      ...
      with autocast(dtype=torch.float16):
        with torch.no_grad():
            ...
            if print_vis_token_meaning:
              prefix_projection = embed[:, q_len:q_len + model.prefix_length, :]
              for i in range(prefix_projection.size(1)):
                print_nearest_text_token(prefix_projection[0, i], model)
            out_text = generate_beam(model, model.tokenizer, generated = embed, entry_length=dataset.max_seq_len[1], temparature=1)[0]
      ...
      a = bert_score.compute(refrences = reference, predictions = candidate, model_type = 'bert-base-uncased' )
