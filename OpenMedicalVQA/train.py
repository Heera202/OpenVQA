...
def pytorch_model_run(trian_loader, valid_loader, model_obj, args):
  ...
  for epoch in range(args.epochs):
    with tqdm(total=args.batch_size * len(train_loader)) as epoch_pbar:
      ...
      for i, (prefix, labels, tokens, mask, q_len) in enumerate(train_loader):
        with accelerator.accumulate(model):
         ...
         shift = 10 if args.setting =="p_tunning" or args.setting == "prompttuning" else 0
    ...
    with tqdm(total=args.batch_size * len(valid_loader)) as epoch_pbar:
      ...
      for i, (prefix, labels, tokens, mask, q_len) in enumerate(valid_loader):
        ...
        with torch.no_grad():
          ouputs = model(prefix, labels, tokens, mask, q_len, batch_size=args.batch_size)
          logits = outputs.logits
          loss = 0.
        ...
        desc = f"VAL Epoch {epoch} - loss {avg_val_loss: .4f}"
        epoch_pbar.set_description(desc)
        epoch_pbar.update(prefix.shape[0])
    ...
    print(" VAL epoch {}/{} \t loss={:.4f} \t val_loss={:4f} \t time={:.2f}s".format(
      epoch + 1, n_epochs, avg_loss, avg_loss, avg_val_loss, elapsed_time
      )
    )
