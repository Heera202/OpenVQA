...
class medvqaDataset(Dataset):
  def __init__(self, path, split='train', like_test=False, prefix_length='2', model_type='gpt2'):
    ...
    self.max_seqs_len = data['max_seq_len']
  ...
  def pad_sequence(self, index):
    text_segments = ['question:', 'context:', 'answer', '<|endoftext|>']
    m = [torch.tensor(self.tokenizer.encode(seg)) for seg in text_segments]
    m_mask = [torch.ones(len(enc)) if i < 3 else torch.zeros(len(enc))
        for i, enc in enumerate([self.tokenizer.encode(seg) for seg in text_segments])]
  ...
  def make_padding(self, max_len, tokens, question=False, leftover_tokens=0):
      ...
      if padding > 0:
        ...
        if question:
          ...
          tokens= torch.zeros(tokens, torch.zeros(padding + leftover_tokens))
          mask = torch.zeros(max_len + leftover_tokens)
  ...
  def __getitem__(self, index):
    ...
    tokens, mask, q_len = self.pad_sequence(index)
